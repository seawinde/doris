...
class CompareOuterJoinTest extends SqlTestBase {
    ...
    void testRandomQuery() {
        ...
        HyperGraph h2 = HyperGraph.builderForMv(p2).buildAll().get(0);
        Assertions.assertFalse(
                HyperGraphComparator.isLogicCompatible(h1, h2, constructContext(p1, p2, planChecker.getCascadesContext())).isInvalid());
    }
    ...
    void testInnerJoinWithFilter2() {
        ...
        HyperGraph h1 = HyperGraph.builderForMv(p1).buildAll().get(0);
        HyperGraph h2 = HyperGraph.builderForMv(p2).buildAll().get(0);
        List<Expression> exprList = HyperGraphComparator.isLogicCompatible(h1, h2, constructContext(p1, p2, c1)).getQueryExpressions();
