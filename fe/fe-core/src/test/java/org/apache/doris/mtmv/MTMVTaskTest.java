...
public class MTMVTaskTest {
    ...
    public void testExecuteIvmAttemptRecordsFallbackReason() throws Exception {
        ...
        try (MockedConstruction<IvmRefreshManager> ignored = Mockito.mockConstruction(IvmRefreshManager.class,
            ...
            Object request = Deencapsulation.invoke(task, "resolveRefreshRequest");
            Object result = Deencapsulation.invoke(task, "executeIvmAttempt", new MTMVRefreshContext(mtmv), request);
    ...
    @Test
    public void testExecuteIvmAttemptRecordsIncrementalProgress() throws Exception {
        Mockito.when(mtmv.isIvm()).thenReturn(true);
        Mockito.when(mtmv.getName()).thenReturn("test_mv");
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        mockNeedIvmRefresh();
        mtmvPartitionUtilStatic.when(() -> MTMVPartitionUtil.generatePartitionSnapshots(
                Mockito.any(MTMVRefreshContext.class), Mockito.anySet(), Mockito.anySet()))
        ...
        Assert.assertEquals(MTMVTask.MTMVTaskRefreshMode.INCREMENTAL,
        ...
        Assert.assertEquals("100.00% (1/1)", Deencapsulation.invoke(task, "getProgress"));
    }

    @Test
    public void testExecuteIvmAttemptRecordsNotRefreshWhenAlreadySynced() throws Exception {
        Mockito.when(mtmv.isIvm()).thenReturn(true);
        Mockito.when(mtmv.getName()).thenReturn("test_mv");
        mtmvPartitionUtilStatic.when(() -> MTMVPartitionUtil.getMTMVNeedRefreshPartitions(
                Mockito.any(MTMVRefreshContext.class), Mockito.anySet())).thenReturn(Lists.newArrayList());
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        ...
        Assert.assertEquals(MTMVTask.MTMVTaskRefreshMode.NOT_REFRESH,
                Deencapsulation.getField(task, "refreshMode"));
        Assert.assertTrue(((List<?>) Deencapsulation.getField(task, "completedPartitions")).isEmpty());
    }
    ...
    @Test
    public void testExecuteIvmAttemptFallsBackToCompleteForBrokenBinlog() throws Exception {
        Mockito.when(mtmv.isIvm()).thenReturn(true);
        Mockito.when(mtmv.getName()).thenReturn("test_mv");
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        mockNeedIvmRefresh();
        ...
        try (MockedConstruction<IvmRefreshManager> ignored = Mockito.mockConstruction(IvmRefreshManager.class,
                (mock, context) -> Mockito.when(mock.doRefresh(mtmv)).thenReturn(
                        IvmRefreshResult.fallback(IvmFailureReason.BINLOG_BROKEN, "broken")))) {
            Object request = Deencapsulation.invoke(task, "resolveRefreshRequest");
            Object result = Deencapsulation.invoke(task, "executeIvmAttempt",
                    new MTMVRefreshContext(mtmv), request);
            Assert.assertEquals("FALLBACK_TO_COMPLETE", result.toString());
        }
    }
    ...
    public void testDebugPlanSignatureDriftFallsBackToFullRefresh() throws Exception {
        ...
        try {
            ...
            try (MockedConstruction<IvmRefreshManager> ignored = Mockito.mockConstruction(IvmRefreshManager.class,
                ...
                Object request = Deencapsulation.invoke(task, "resolveRefreshRequest");
                mockNeedIvmRefresh();
                Object result = Deencapsulation.invoke(task, "executeIvmAttempt", new MTMVRefreshContext(mtmv),
                        request);
                Assert.assertEquals("FALLBACK_TO_COMPLETE", result.toString());
            }
    ...
    @Test
    public void testFullRefreshPublishesCapturedIvmPlanSignature() throws Exception {
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        ...
        Deencapsulation.setField(task, "ivmFallbackPlanSignature", signature);

        try (MockedStatic<IvmRefreshManager> managerStatic = Mockito.mockStatic(IvmRefreshManager.class)) {
            Deencapsulation.invoke(task, "finishIvmFullRefreshIfNeeded", true, 7L);

            managerStatic.verify(() -> IvmRefreshManager.finishIvmFullRefresh(mtmv, 7L, signature));
        }
    }

    @Test
    public void testCompleteRefreshClearsBrokenBinlogAfterStreamReset() throws Exception {
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        Deencapsulation.setField(task, "refreshMode", MTMVTask.MTMVTaskRefreshMode.COMPLETE);
        ...
        try (MockedStatic<IvmRefreshManager> managerStatic = Mockito.mockStatic(IvmRefreshManager.class)) {
            ...
            managerStatic.verify(() -> IvmRefreshManager.finishIvmFullRefresh(mtmv, 7L, null));
        }
    }

    @Test
    public void testSignatureMismatchFullRefreshRequiresCapturedSignature() throws Exception {
        Mockito.when(mtmv.getName()).thenReturn("test_mv");
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        Deencapsulation.setField(task, "refreshMode", MTMVTask.MTMVTaskRefreshMode.COMPLETE);
        Deencapsulation.setField(task, "ivmFallbackReason", IvmFailureReason.PLAN_SIGNATURE_MISMATCH.name());
        ...
        try (MockedStatic<IvmRefreshManager> managerStatic = Mockito.mockStatic(IvmRefreshManager.class)) {
            ...
            Assert.assertTrue(exception.getMessage().contains("Missing current IVM plan signature"));
            managerStatic.verifyNoInteractions();
        }
    }

    @Test
    public void testExplicitFullPartitionRefreshDoesNotClearBrokenBinlog() throws Exception {
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        Deencapsulation.setField(task, "refreshMode", MTMVTask.MTMVTaskRefreshMode.COMPLETE);

        try (MockedStatic<IvmRefreshManager> managerStatic = Mockito.mockStatic(IvmRefreshManager.class)) {
            Deencapsulation.invoke(task, "finishIvmFullRefreshIfNeeded", false, -1L);
    ...
    @Test
    public void testExplicitPartitionRefreshMarksIvmBaselineBroken() throws Exception {
        Mockito.when(mtmv.isIvm()).thenReturn(true);
        MTMVTaskContext context = MTMVTaskContext.of(MTMVTaskTriggerMode.MANUAL,
                Lists.newArrayList(poneName), RefreshMode.PARTITIONS, false, null);
        MTMVTask task = new MTMVTask(mtmv, relation, context);
        Object request = Deencapsulation.invoke(task, "resolveRefreshRequest");

        try (MockedStatic<IvmRefreshManager> managerStatic = Mockito.mockStatic(IvmRefreshManager.class)) {
            boolean useIvmFallbackStreams = Deencapsulation.invoke(
                    task, "shouldUseIvmFallbackStreams", request);

            Assert.assertFalse(useIvmFallbackStreams);
            managerStatic.verify(() -> IvmRefreshManager.markIvmBaselineBroken(mtmv));
        }
    }

    @Test
    public void testIvmFallbackStreamScanContextModes() throws Exception {
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        BaseTableInfo nonPctTable = new BaseTableInfo(new TableNameInfo("ctl", "db1", "non_pct"));
        BaseTableInfo pctTable = new BaseTableInfo(new TableNameInfo("ctl", "db2", "pct"));
        Object nonPctCapture = newIvmFallbackCapture(nonPctTable, 101L);
        Object batchCapture = newIvmFallbackCapture(pctTable, 201L);

        Deencapsulation.setField(task, "refreshMode", MTMVTask.MTMVTaskRefreshMode.COMPLETE);
        Map<BaseTableInfo, IvmFallbackStreamScanContext> firstBatchContexts = Deencapsulation.invoke(task,
                "buildIvmFallbackStreamScanContexts", nonPctCapture, batchCapture, true);
        Assert.assertEquals(IvmFallbackStreamScanMode.RESET, firstBatchContexts.get(nonPctTable).getScanMode());
        Assert.assertEquals(IvmFallbackStreamScanMode.RESET, firstBatchContexts.get(pctTable).getScanMode());
        Assert.assertEquals(100L,
                firstBatchContexts.get(nonPctTable).getCapturedUpdate().getNext().get(101L).longValue());

        Map<BaseTableInfo, IvmFallbackStreamScanContext> secondBatchContexts = Deencapsulation.invoke(task,
                "buildIvmFallbackStreamScanContexts", nonPctCapture, batchCapture, false);
        Assert.assertEquals(IvmFallbackStreamScanMode.SNAPSHOT, secondBatchContexts.get(nonPctTable).getScanMode());
        Assert.assertEquals(IvmFallbackStreamScanMode.RESET, secondBatchContexts.get(pctTable).getScanMode());

        Deencapsulation.setField(task, "refreshMode", MTMVTask.MTMVTaskRefreshMode.PARTIAL);
        Map<BaseTableInfo, IvmFallbackStreamScanContext> partialContexts = Deencapsulation.invoke(task,
                "buildIvmFallbackStreamScanContexts", nonPctCapture, batchCapture, true);
        Assert.assertFalse(partialContexts.containsKey(nonPctTable));
        Assert.assertEquals(IvmFallbackStreamScanMode.RESET, partialContexts.get(pctTable).getScanMode());
    }

    @Test
    public void testCaptureIncludesNonEmptyPartitionWithoutStreamLag() {
        MTMVTask task = new MTMVTask(mtmv, relation, new MTMVTaskContext(MTMVTaskTriggerMode.MANUAL));
        Partition partition = Mockito.mock(Partition.class);
        Mockito.when(partition.hasData()).thenReturn(true);
        Mockito.when(partition.getId()).thenReturn(101L);
        Mockito.when(partition.getTso()).thenReturn(200L);
        OlapTableStream stream = Mockito.mock(OlapTableStream.class);
        Mockito.when(stream.getStreamUpdate(101L)).thenReturn(Pair.of(200L, 200L));

        Map<Long, Long> partitionTsoById = new HashMap<>();
        Map<Long, Long> prev = new HashMap<>();
        Map<Long, Long> next = new HashMap<>();
        Deencapsulation.invoke(task, "capturePartitionState", partition, stream,
                partitionTsoById, prev, next);

        Assert.assertEquals(Long.valueOf(200L), partitionTsoById.get(101L));
        Assert.assertEquals(Long.valueOf(200L), prev.get(101L));
        Assert.assertEquals(Long.valueOf(200L), next.get(101L));
    }
    ...
    public void testIvmExecutionFailureDoesNotFallback() throws Exception {
        ...
        try (MockedConstruction<IvmRefreshManager> ignored = Mockito.mockConstruction(IvmRefreshManager.class,
            ...
            Object request = Deencapsulation.invoke(task, "resolveRefreshRequest");
            JobException exception = Assert.assertThrows(JobException.class,
                    () -> Deencapsulation.invoke(task, "executeIvmAttempt", new MTMVRefreshContext(mtmv), request));
    ...
    private void mockNeedIvmRefresh() throws AnalysisException {
        mtmvPartitionUtilStatic.when(() -> MTMVPartitionUtil.getMTMVNeedRefreshPartitions(
                Mockito.nullable(MTMVRefreshContext.class), Mockito.nullable(Set.class)))
                .thenReturn(Lists.newArrayList(poneName));
    }

    private Object newIvmFallbackCapture(BaseTableInfo baseTableInfo, long partitionId)
            throws Exception {
        Object capture = Deencapsulation.newInstance(
                Class.forName(MTMVTask.class.getName() + "$IvmFallbackCapture"));
        Map<BaseTableInfo, OlapTableStreamUpdate> streamUpdates =
                Deencapsulation.getField(capture, "streamUpdates");
        Map<Long, Long> next = new HashMap<>();
        next.put(partitionId, 100L);
        streamUpdates.put(baseTableInfo, new OlapTableStreamUpdate(new HashMap<>(), next));
        return capture;
    }
